...
void Scanner::getNextToken() {
    ...
    if (scannerIndex == 1 && (this->currentState & TOKEN_BIT)) {
    	...
    	case '(': this->generateToken(Token::TT_BRACKET_UPON);
    	    break;
    	case ')': this->generateToken(Token::TT_BRACKET_CLOSE);
    	    break;
		...
		this->init();
		//this->automat->incrementColCount(); TODO
    } else {
		if (this->currentState & TOKEN_BIT) {

			this->scannerIndex--;
			//Col COunt ebenfalls zurücksetzen
			...
			if (this->commentFlag == 0) {
				if (automat->characterType(this->internBuffer[0]) == Automat::CT_NUMERIC) {
					...
					if(automat->characterType(this->internBuffer[0]) == Automat::CT_SIGN_LESS){
						if (automat->characterType(this->internBuffer[1]) == Automat::CT_SIGN_DOUBLE) {
							//<:>
				...
				if((automat->characterType(this->internBuffer[0]) == Automat::CT_SLASH) &&
						(automat->characterType(this->internBuffer[1]) != Automat::CT_STAR)){
					// /-Token
					this->generateToken(Token::TT_SLASH);
				} else {
			...
			//Col Count ebenfalls um gleichen Wert zurücksetzen
...
void Scanner::generateToken(int typ) {
	token = new Token(this->automat->getRows(), this->automat->getCols() - (this->scannerIndex - 1), typ, this->internBuffer);
...
void Scanner::printErrorChar() {
	//printf("unbekanntes Token Zeile: %d \tSpalte: %d \tSymbol: %c\n", this->automat->getRows(), this->automat->getCols() ,this->internBuffer[0]);
	fprintf(stderr, "unbekanntes Token Zeile: %d \tSpalte: %d \tSymbol: %c\n", this->automat->getRows(), this->automat->getCols() ,this->internBuffer[0]);
}
